...
public class Tree_Child : MonoBehaviour {
    ...
    public int indexInInstantiatedChildrenList; //use this to connect Tree_Child to its corresponding contents[i]
    public Vector3 assignedSplaySPLocation;
    public Vector3 parentContainerPos;
    private Vector3 parentPosPlusUP;
    ...
    void Start() {
        ...
        //parentContainerPos = transform.parent.transform.position;
        ...
        //TRY PARENTING SP
        //instantiatedStartpoint.transform.SetParent(gameObject.transform); //adds to the end of contents?



        //SET START POINT
        //SET UPDATE POINT -- THIS IS WHAT I'M UPDATING EVERY FRAME
        ////now start point is being set by parent tree
    ...
    void Update() {
        ...
        //parentContainerPos = transform.parent.transform.position;
        //parentPosPlusUP = updatePoint + parentContainerPos;

        Splay(updatePoint);
        //Splay(parentPosPlusUP);
        ...
        Debug.Log("my assigned splay sp loc is: " + assignedSplaySPLocation);
        Debug.Log("UP loc is: " + updatePoint);
        //Debug.Log("container position: " + this.GetComponentInParent<Transform>().position);
    ...
    */
    ...
    void Splay(Vector3 updatePoint) {
        ...
        distanceVector = updatePoint - assignedSplaySPLocation;

        //Debug.Log("child startPoint = " + startPoint);

        segmentVector = distanceVector / (contentsCount + 1);  //+1?

        //i was using startPoint.z as distance mark -- n00b :)
        float kidScale = Vector3.Distance(updatePoint, assignedSplaySPLocation) * scaleMult;
        ...
        for (int i = 0; i < contents.Length; i++) {
            ...
            if (!skipContentsIndices.Contains(i)) {
                ...
                //float newZ = segmentVector.z * (i + 1) * interpMultiplier;
                //float newY = segmentVector.y * (i + 1) * interpMultiplier;
                //float newX = segmentVector.x * (i + 1) * interpMultiplier;

                //update child positions
                if (!LookAt) {
                    //iTween.MoveUpdate(kid, iTween.Hash("z", newZ, "y", newY, "x", newX, "islocal", true, "time", 0.7));

                    //always doing LookAt update point bc files were pointing incorrectly without it
